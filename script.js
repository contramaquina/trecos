function pergaminhoAleatorio() {
    ...
    try {
      ...
      if (nivel >= 1 && nivel <= 9) {
        pergaminhosEscolhidos = dicionarioMagias[`magias_nivel_${nivel}`].slice(0, quantidade);
        pergaminhosEscolhidos = pergaminhosEscolhidos.map(
          (magia) => `- ${magia}`
        );
        const mensagem =
          `Pergaminhos de ${nivel}° nível gerados:\n\n` +
